...
function numpad_create( fn_exit, fn_channel )
{
	...
	$(document.body).append('\
		<div id="numpad-cntnr" class="numpad-cntnr-brdr numpad-bkg">\
			<div>\
				<div id="npad-chnn-win"      class="npad-btn-brdr numpad-bkg"><span id="npad-channum-txt">0</span></div>\
	...
	{
		$_numpad.hide( os1_is_small_device() ? 0 : 300 );
		fn_exit();
		return false;
	} ).mouseenter( function()
	{
...
function numpad_size_and_offset_set( height, l_top, l_left )
{
	...
	$( "#npad-chnn-win" ).height( hei ).width( wid );
...
function numpad_size_and_offset_set_center( height, par_top, par_left, par_height, par_width )
{
	numpad_size_set( height );
	numpad_size_and_offset_set( height, par_top + ((par_height - $_numpad.height()) / 2), par_left + ((par_width - $_numpad.width()) / 2) );
}

function numpad_show( current_channel, is_volatile, close_enter )
{
	...
	$_numpad.show( os1_is_small_device() ? 0 : 300 );
}

function numpad_hide()
{
	$_numpad.hide( os1_is_small_device() ? 0 : 300 );
}
