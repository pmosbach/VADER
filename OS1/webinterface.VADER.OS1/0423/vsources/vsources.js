...
function vsources_create( fn_status, fn_exit, fn_volume, fn_power, fn_vsrc_click, fn_$_connected_to )
{
	...
	for( var sx = 0; sx < $_vsources.length; sx++ )
	{
		...
		{
			fn_vsrc_click ( $(this) );
			$_vsources_cntnr.hide( os1_is_small_device() ? 0 : 300 );
			return false;
		} ).mouseenter( function()
		{
...
function vsources_size_and_offset_set_center( height, par_top, par_left, par_height, par_width )
{
	vsrc_size_set( height );
	vsources_size_and_offset_set( height, par_top + ((par_height - $_vsources_cntnr.height()) / 2), par_left + ((par_width - $_vsources_cntnr.width()) / 2) );
}
...
function vsources_show( headermsg, a_hevs_is_selected, $vsrc_connected_to )
{
	...
	$_vsrc_headr.html( "<b>" + headermsg + "</b>" );

	$_vsources_cntnr.show( os1_is_small_device() ? 0 : 300 );
}

function vsources_hide()
{
	$_vsources_cntnr.hide( os1_is_small_device() ? 0 : 300 );
}
