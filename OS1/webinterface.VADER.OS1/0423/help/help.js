...
function help_create( fn_exit )
{
	...
	$_help_btn_exit = $( "#help-btn-x" ).click( function()   // EXIT
	{
		...
		$_help.hide( os1_is_small_device() ? 0 : 300 );
		fn_exit();
		return false;
	} ).mouseenter( function()
	{
...
function help_size_and_offset_set( height, l_top, l_left )
{
	...
	height_max = height;
	posn_sgl  = { top:l_top, left:l_left/2 };
	posn_mlt  = { top:l_top, left:l_left/32 };
	posn_pwr  = { top:l_top + (height/4), left:l_left };
	posn_chn  = { top:l_top + (height/8), left:l_left/4 };
	posn_dflt = { top:l_top + (height/8), left:l_left/4 };
}

function help_show()
{
	$_help.show( os1_is_small_device() ? 0 : 300 );
}
...
{
	$_help_size_and_offset_set( $_help_sgl, height_max, (876/512), posn_sgl.top, posn_sgl.left );
}
...
{
	$_help_size_and_offset_set( $_help_mlt, height_max, (876/512), posn_mlt.top, posn_mlt.left );
}
...
{
	$_help_size_and_offset_set( $_help_pwr, height_max * 0.60, (876/512), posn_pwr.top, posn_pwr.left );
}
